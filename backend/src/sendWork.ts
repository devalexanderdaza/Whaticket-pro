...
const sendMessage = async (whatsappId: number, photo: boolean) => {
  ...
  if (photo && !isContactPhoto) {
    // logger.warn(`${getMessage.phone} - não tem foto, não enviar por que o contato não tem foto`);
    await getMessage.update({
  ...
  if (photo && isContactPhoto) {
    // logger.info(`${getMessage.phone} - fazendo o envio para o whatsapp para usuário com e sem foto!`);
...
const processMessage = async (whatsappId: number) => {
  ...
  if (!setting) {
    // logger.info(`Configuração de envio não encontrada da conexão com id ${setting.whatsappId}`);
    return;
  }
  ...
  if (!limit) {
    // logger.warn(`conexão sem limite configurado, envio parado. ${setting.whatsappId}`);

    return;
  }

  // logger.info(`limit: ${limit} minutes: ${minutes} seconds: ${seconds} photo: ${photo} random: ${random}`);
  ...
  if (!intervalExist) {
    const interval = setInterval(async () => {
      // logger.info(`Iniciando a conexao com id ${whatsappId} as - ${new Date()}`);
...
export const runSendMessage = async (): Promise<void> => {
  if (!jobWork) {
    ...
    // logger.info(`Total de conexao Prontas para envio!: ${whatsappConnected.length}`);
  }
};
